...
double System :: Force(int i, int dim){ // Calculates the force acting on particle i along dimension dim
  double f=0.0, dr;
  vec distance;
  ...
  for (int j=0; j<_npart; j++){
    if(i != j){ // Avoid self-interaction
      ...
      if(dr < _r_cut){
        // Derivative of the Lennard-Jones potential along dimension dim
...
// ====================================================
// MOVE FUNCTION AND METROPOLIS ALGORITHM
// ====================================================
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  if(_sim_type==0) this->initialize_velocities(); // Called only if the simulation is Molecular Dynamics
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    cinf.close();
  } else { // TO BE TEMPORARILY CHANGED IN EXERCISE 4.2 (CHANGE THE GAUSSIANS IN DIRAC DELTA)
    vec vx(_npart), vy(_npart), vz(_npart);
    vec sumv(_ndim);
    sumv.zeros();
    for (int i=0; i<_npart; i++){
      // EXERCISE 4.1) SAMPLING VELOCITY COMPONENTS FROM GAUSSIAN DISTRIBUTION (speed modules follow Maxwell-Boltzmann distribution)
      // COMMENT THE FOLLOWING LINES FOR EXERCISE 4.2 AND 4.3
...
//      vz(i) = 0.0; 
    ...
    sumv2 /= double(_npart);
    // Rescale speed to simulate correct temperature (see analysis4.ipynb for details)
    ...
    // Velocities are needed to calculate old positions to initialize Verlet algorithm
    for (int i=0; i<_npart; i++){
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        // ================================================================================================================
        // EXERCISE 7.1) TAIL CORRECTION FOR POTENTIAL ENERGY
        ...
        index_property++;
      } else if( property == "TEMPERATURE" ){ // If temperature is being measure, we need to also calculate kinetic energy
        ...
        index_property++;
        // ================================================================================================================
        // EXERCISE 7.1) TAIL CORRECTION FOR PRESSURE
        _ptail = 32.0 * M_PI * _rho * (1.0/(9.0 * pow(_r_cut, 9)) - 1.0/(6.0 * pow(_r_cut, 3))); // TO BE FIXED IN EXERCISE 7
        // ================================================================================================================
      ...
      } else if( property == "POFV" ){ // Distribution of particle velocities (speed modules at equilibrium follow MB distribution)
        if(_sim_type > 0){
        ...
        _nprop += _n_bins_v;
        // ================================================================================================================
        // TO BE FIXED IN EXERCISE 4 (CHANGED FROM 4.0 TO 5.0 TO BETTER SAMPLE THE HIGH VELOCITY TAIL OF THE DISTRIBUTION)
        _bin_size_v = 5.0*_temp/(double)_n_bins_v; 
        // ================================================================================================================
        _measure_pofv = true;
...
void System :: averages(int blk){
  ...
  // POFV ///////////////////////////////////////////////////////
  // ============================================================================================================================
  // FOR EXERCISE 4.1 (Gaussian sampling of velocities) I plot the progresive average result of the last block
  // COMMENT FOR EXERCISE 4.2 AND 4.3
  if (_measure_pofv){

    // Only print last block average
    if(blk == _nblocks){
  ...
  // ============================================================================================================================
  // UNCOMMENT THE FOLLOWING BLOCK FOR EXERCISE 4.2 (Delta sampling of velocities)
