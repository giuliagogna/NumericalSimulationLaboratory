...
_**Note on Large Data Files:** The standard output log containing the complete evolutionary history of the Italian provincial capitals exceeds GitHub's file size limit (100 MB) and is deliberately excluded from this repository. However, a lightweight alternative, `outputs/poplog_N110_Pop700_Gen2000_Italy_synth.dat`, is included. This "synthetic" log preserves the exact numerical metadata (Generation, Rank, Fitness) for the entire population, but only stores the full geometric path for the absolute best individual of each generation. This allows the Jupyter Notebook to visualize all essential statistical metrics and spatial results without clogging the repository. (If you wish to verify the full real-world map data, you can easily reproduce it locally by setting USE_SYNTHETIC_LOG 0 in inputs/input.dat and executing the code)._