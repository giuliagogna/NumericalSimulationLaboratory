...
void Population::ExchangeBest(const vector<int>& foreign_tour) {
    
    // Overwite the best individual of the population with the foreign individual
    _pop[0] = Individual(foreign_tour, _coords);
    
    // Resort the population so the foreigner gets in the correct position according to its fitness
    SortByFitness(); 
}